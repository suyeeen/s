...
@section('content')
    <div class="max-w-6xl mx-auto space-y-8">
        ...
        <div>
            ...
            <p class="mt-2 text-sm" style="color: var(--text-muted)">Pantau aktivitas AI K-Means Clustering dan status
                operasional sistem STQM.</p>
        </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    @foreach ($clusterConfig as $key => $cfg)
                        <div class="rounded-2xl p-5 text-center"
                            ...
                            <div class="text-3xl font-black my-2" style="color: {{ $cfg['color'] }}">
                                {{ $clusterDistribusi[$key] }}</div>
                            ...
                            @if($sudahDicluster > 0)
                                <div class="mt-3 h-1.5 rounded-full" style="background: rgba(255,255,255,0.06);">
                                    <div class="h-1.5 rounded-full"
                                        style="background: {{ $cfg['color'] }}; width: {{ round(($clusterDistribusi[$key] / $sudahDicluster) * 100) }}%;">
                                    </div>
                                </div>
                                <div class="text-xs mt-1" style="color: var(--text-muted)">
                                    {{ $sudahDicluster > 0 ? round(($clusterDistribusi[$key] / $sudahDicluster) * 100) : 0 }}%</div>
                            @endif
                        </div>
                    @endforeach
                </div>
            ...
            <div class="rounded-3xl p-6 flex flex-col justify-between"
                ...
                <div>
                    ...
                    <p class="text-xs mb-6" style="color: var(--text-muted)">
                        Algoritma K-Means akan memproses semua jawaban kuesioner dan memetakan setiap guru ke dalam 4
                        klaster berdasarkan kompetensi pedagogik, profesional, sosial, dan kepribadian.
                    </p>
                    <div class="space-y-3 text-xs" style="color: var(--text-muted)">
                        <div class="flex items-center gap-2">
                            <span
                                ...
                                style="background: rgba(139,92,246,0.4);">1</span>
                            Ambil jawaban kuesioner dari database
                        </div>
                        <div class="flex items-center gap-2">
                            <span
                                class="w-5 h-5 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0"
                                style="background: rgba(139,92,246,0.4);">2</span>
                            Hitung rata-rata nilai per kompetensi
                        </div>
                        <div class="flex items-center gap-2">
                            <span
                                class="w-5 h-5 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0"
                                style="background: rgba(139,92,246,0.4);">3</span>
                            Proses Python K-Means (4 klaster)
                        </div>
                        <div class="flex items-center gap-2">
                            <span
                                class="w-5 h-5 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0"
                                style="background: rgba(139,92,246,0.4);">4</span>
            ...
            <div class="lg:col-span-2 rounded-3xl overflow-hidden"
                ...
                <div class="px-6 py-5"
                    ...
                    <h3 class="font-semibold text-sm flex items-center gap-2" style="color: var(--text-main)">
                        <svg class="w-4 h-4" fill="none" stroke="#fbbf24" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
