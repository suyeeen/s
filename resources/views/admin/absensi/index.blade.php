...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    {{-- Alert sukses/error --}}
    @if(session('success'))
        <div class="px-4 py-3 rounded-xl text-sm font-semibold" style="background:rgba(16,185,129,0.12); color:#10b981; border:1px solid rgba(16,185,129,0.2);">
            {{ session('success') }}
        </div>
    @endif
    @if(session('error'))
        <div class="px-4 py-3 rounded-xl text-sm font-semibold" style="background:rgba(239,68,68,0.12); color:#ef4444; border:1px solid rgba(239,68,68,0.2);">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div id="modalImport" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="w-full max-w-lg rounded-2xl p-6 space-y-4" style="background:var(--card-bg); border:1px solid var(--card-border);">
            <div class="flex items-center justify-between">
                ...
                <button onclick="document.getElementById('modalImport').classList.add('hidden')"
                        class="w-8 h-8 rounded-lg flex items-center justify-center" style="color:var(--text-muted); background:var(--card-bg-soft);">✕</button>
            </div>

            <div class="rounded-xl px-4 py-3 text-xs leading-relaxed" style="background:rgba(59,130,246,0.08); border:1px solid rgba(59,130,246,0.2); color:#3b82f6;">
            ...
            <form method="POST" action="{{ route('admin.absensi.import.preview') }}" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-1 py-2 rounded-xl font-bold text-sm text-white" style="background:#10b981;">
                        Validasi & Preview
                    </button>
    ...
    @if(session('import_preview') || session('import_log_gagal'))
        ...
        <div class="rounded-2xl p-5 space-y-4" style="background:var(--card-bg); border:1px solid rgba(16,185,129,0.3);">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" style="background:rgba(16,185,129,0.15);">
                            ...
                            @foreach($preview as $p)
                                ...
                                <tr style="border-bottom:1px solid var(--card-border-soft);">
                                    ...
                                    <td class="px-3 py-2 text-center">
                                        @php $pct = $p['persen']; $pc = $pct >= 90 ? '#10b981' : ($pct >= 75 ? '#f59e0b' : '#ef4444'); @endphp
    ...
    <div class="rounded-2xl p-5" style="background:var(--card-bg); border:1px solid var(--card-border);">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            @foreach($guruList as $g)
                @php $pct = $g['persen_hadir']; $color = $pct >= 90 ? '#10b981' : ($pct >= 75 ? '#f59e0b' : '#ef4444'); @endphp
                <div class="rounded-xl p-3 text-center" style="background:var(--card-bg-soft); border:1px solid var(--card-border-soft);">
    ...
    <form method="GET" class="flex flex-wrap gap-3">
        <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama / NIP..."
               class="rounded-xl px-3 py-2 text-sm" style="background:var(--card-bg); border:1px solid var(--card-border); color:var(--text-main); min-width:200px;">
        <select name="bulan" class="rounded-xl px-3 py-2 text-sm" style="background:var(--card-bg); border:1px solid var(--card-border); color:var(--text-main);">
        ...
        <input type="number" name="tahun" value="{{ $tahunFilter }}" placeholder="Tahun"
               class="rounded-xl px-3 py-2 text-sm w-24" style="background:var(--card-bg); border:1px solid var(--card-border); color:var(--text-main);">
        <button type="submit" class="px-4 py-2 rounded-xl text-sm font-bold text-white" style="background:var(--accent);">Filter</button>
    ...
    <div class="rounded-2xl overflow-hidden" style="background:var(--card-bg); border:1px solid var(--card-border);">
        <table class="w-full text-sm">
            <thead>
                <tr style="border-bottom:1px solid var(--card-border-soft);">
                    <th class="text-left px-4 py-3 font-bold text-xs uppercase tracking-wider" style="color:var(--text-muted);">Guru</th>
                    <th class="text-center px-4 py-3 font-bold text-xs uppercase tracking-wider" style="color:var(--text-muted);">Periode</th>
                    <th class="text-center px-4 py-3 font-bold text-xs uppercase tracking-wider" style="color:#10b981;">Hadir</th>
                    <th class="text-center px-4 py-3 font-bold text-xs uppercase tracking-wider" style="color:#f59e0b;">Izin</th>
                    <th class="text-center px-4 py-3 font-bold text-xs uppercase tracking-wider" style="color:#3b82f6;">Sakit</th>
                    <th class="text-center px-4 py-3 font-bold text-xs uppercase tracking-wider" style="color:#ef4444;">Alpha</th>
                    <th class="text-center px-4 py-3 font-bold text-xs uppercase tracking-wider" style="color:#a855f7;">Terlambat</th>
                    <th class="text-center px-4 py-3 font-bold text-xs uppercase tracking-wider" style="color:var(--text-muted);">% Hadir</th>
                    <th class="text-center px-4 py-3 font-bold text-xs uppercase tracking-wider" style="color:var(--text-muted);">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @forelse($query as $rekap)
                    @php $pct = $rekap->persen_hadir; $color = $pct >= 90 ? '#10b981' : ($pct >= 75 ? '#f59e0b' : '#ef4444'); @endphp
                    <tr style="border-bottom:1px solid var(--card-border-soft);">
                        <td class="px-4 py-3 font-semibold" style="color:var(--text-main);">
                            {{ $rekap->guru->nama ?? '—' }}
                            <p class="text-xs font-normal" style="color:var(--text-muted);">{{ $rekap->guru->nip ?? '' }}</p>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="px-2 py-1 rounded-lg text-xs font-black" style="background:{{ $color }}22; color:{{ $color }};">
