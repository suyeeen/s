...
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <form method="POST" action="{{ route('admin.settings.save') }}" id="settingsForm">
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-3xl overflow-hidden" style="background:var(--card-bg);border:1px solid var(--card-border);box-shadow:0 1px 8px rgba(26,22,19,0.05);">
                <div class="p-5 flex items-center gap-4" style="border-bottom:1px solid var(--card-divider);background:var(--card-bg-soft);">
                    ...
                    <div>
                        ...
                        <p class="text-xs mt-0.5" style="color:var(--text-muted)">Atur tahun ajaran aktif dan semester berjalan.</p>
                    </div>
                </div>
            ...
            <div class="rounded-3xl overflow-hidden" style="background:var(--card-bg);border:1px solid var(--card-border);box-shadow:0 1px 8px rgba(26,22,19,0.05);">
                <div class="p-5 flex items-center justify-between gap-4" style="border-bottom:1px solid var(--card-divider);background:var(--card-bg-soft);">
                    <div class="flex items-center gap-4">
                        <div class="p-2.5 rounded-xl" style="background:rgba(79,70,229,0.1);border:1px solid rgba(79,70,229,0.2);">
                            <svg class="w-5 h-5" style="color:#6366f1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                        </div>
                        <div>
                            <h3 class="font-semibold text-base" style="color:var(--text-main)">Jadwal Buka / Tutup Kuesioner</h3>
                            <p class="text-xs mt-0.5" style="color:var(--text-muted)">Kontrol kapan siswa dan guru bisa mengisi kuesioner.</p>
                        </div>
                    </div>
                    {{-- Status Badge --}}
                    <div class="px-4 py-2 rounded-2xl text-xs font-bold flex items-center gap-2 shrink-0"
                        ...
                        <span class="w-2 h-2 rounded-full" style="background:{{ $statusColor }};box-shadow:0 0 6px {{ $statusColor }};"></span>
                        {{ $statusLabel }}
                    </div>
                </div>
                ...
                <div class="p-7 space-y-6">
                    ...
                    {{-- Status detail --}}
                    <div class="p-4 rounded-2xl" style="background:{{ $statusBg }};border:1px solid {{ $statusBorder }};">
                        <div class="flex items-start gap-3 text-sm" style="color:{{ $statusColor }}">
                            ...
                            <div class="font-medium">
                                ...
                                @elseif($buka && $now < $buka)
                                    Kuesioner belum dibuka. Akan dibuka pada <strong>{{ \Carbon\Carbon::parse($buka)->isoFormat('D MMMM Y') }}</strong>
                                    — {{ $sisaHari }} hari lagi.
                                @elseif($tutup && $now > $tutup)
                                    Kuesioner sudah ditutup sejak <strong>{{ \Carbon\Carbon::parse($tutup)->isoFormat('D MMMM Y') }}</strong>.
                                    Siswa dan guru tidak dapat mengisi kuesioner.
                                @else
                                    Kuesioner sedang terbuka.
                                    @if($buka) Dibuka sejak <strong>{{ \Carbon\Carbon::parse($buka)->isoFormat('D MMMM Y') }}</strong>.@endif
                                    @if($tutup) Akan ditutup pada <strong>{{ \Carbon\Carbon::parse($tutup)->isoFormat('D MMMM Y') }}</strong>
                    ...
                    {{-- Quick-set presets --}}
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-wide mb-3" style="color:var(--text-muted)">Preset Cepat</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="mt-1.5 text-xs" style="color:var(--text-muted)">Kosongkan = tidak dibatasi dari awal.</p>
                        </div>
                        <div>
                            <label class="block text-xs font-semibold mb-2 uppercase tracking-wide" style="color:var(--text-muted)">
                            ...
                            <input type="date" name="tutup_kuesioner" id="inputTutup"
                                ...
                                style="background:var(--input-bg);border:1.5px solid var(--input-border);color:var(--text-main);"
                                oninput="validateJadwal()">
                            <p class="mt-1.5 text-xs" style="color:var(--text-muted)">Kosongkan = tidak dibatasi dari akhir.</p>
                        </div>
                    </div>
