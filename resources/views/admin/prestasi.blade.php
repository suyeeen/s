                            ...
                            @forelse ($prestasi_menunggu as $item)
                                <tr style="border-bottom:1px solid var(--card-border-soft);"
                                    ...
                                    <td class="p-5">
                                        @if ($item->file_bukti)
                                            @php
                                                $ext = strtolower(pathinfo($item->file_bukti, PATHINFO_EXTENSION));
                                                $isPdf = $ext === 'pdf';
                                                $buktUrl = \Illuminate\Support\Facades\Storage::disk('public')->exists($item->file_bukti)
                                                    ? \Illuminate\Support\Facades\Storage::url($item->file_bukti)
                                                    : null;
                                            @endphp
                                            @if($buktUrl)
                                                <a href="{{ $buktUrl }}" target="_blank" rel="noopener noreferrer"
                                                    ...
                                                    @endif
                                                    Lihat Bukti {{ strtoupper($ext) }}
                                                </a>
                                            @else
                                                <span class="text-xs" style="color:#f87171;">
