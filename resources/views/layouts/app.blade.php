...
<head>
    ...
    <style>
        ...
        .theme-toggle:hover { opacity: .75; transform: rotate(18deg); }
...
<body class="min-h-screen" x-data="{ sidebarOpen: false }">
    <div class="flex min-h-screen">
        ...
        <div x-show="sidebarOpen" x-transition.opacity
            class="fixed inset-0 bg-black/50 z-30 md:hidden" @click="sidebarOpen = false"></div>
        ...
        <main class="flex-1 flex flex-col">
            <header class="md:hidden flex items-center justify-between p-4 border-b backdrop-blur">
                ...
                <div class="flex items-center gap-2">
                    <button onclick="toggleTheme()" class="theme-toggle w-9 h-9 rounded-xl flex items-center justify-center">
                        <svg id="app-icon-sun" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z"/>
                        </svg>
                        <svg id="app-icon-moon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor" class="w-4 h-4 hidden">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            Swal.fire({ icon:'success', title:'Berhasil!', text:'{{ addslashes(session("success")) }}',
        ...
        document.addEventListener('DOMContentLoaded', function () {
            const t = getSwalTheme();
            Swal.fire({ icon:'error', title:'Gagal!', text:'{{ addslashes(session("error")) }}',
                confirmButtonColor:'#E8560A', background:t.background, color:t.color,
                customClass:{ popup:'swal2-popup' } });
        });
        @endif
        ...
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('.swal-delete').forEach(function (form) {
                form.addEventListener('submit', function (e) {
                    ...
                    const t = getSwalTheme();
                    Swal.fire({ icon:'warning', title:'Hapus '+nama+'?',
            ...
            document.querySelectorAll('.swal-confirm').forEach(function (btn) {
                btn.addEventListener('click', function (e) {
                    ...
                    const t = getSwalTheme();
                    Swal.fire({ icon:'question', title:judul, text:pesan,
