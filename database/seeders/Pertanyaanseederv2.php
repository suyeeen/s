 ...
 *          Profesional Guru (9).
 *
 * CATATAN: Seeder ini TIDAK menghapus data lama secara otomatis.
 *          Jalankan dengan perintah:
 *              php artisan db:seed --class=PertanyaanSeederV2
 *          Jika ingin mengganti data lama, tambahkan flag --force dan
 *          pastikan sudah melakukan backup terlebih dahulu.
 * ══════════════════════════════════════════════════════════════════════════════
 */
class PertanyaanSeederV2 extends Seeder
{
    public function run(): void
    {
        ...
        $this->command->info('');
        $this->command->info('✅ PertanyaanSeederV2 — ' . count($pertanyaan) . ' pertanyaan berhasil di-seed.');
        $this->command->info('');
        $this->command->table(
            ['Penilai', 'Kategori / Komponen', 'V1 (lama)', 'V2 (baru)', 'Sumber'],
            [
                ['Siswa', 'pedagogik    (KI 1–10)',          '10 soal', '4 soal', 'Permendiknas 16/2007 — adaptasi'],
                ['Siswa', 'kepribadian  (KI 11–15)',         ' 6 soal', '3 soal', 'Permendiknas 16/2007 — adaptasi'],
                ['Siswa', 'sosial       (KI 16–19)',         ' 4 soal', '2 soal', 'Permendiknas 16/2007 — adaptasi'],
                ['Siswa', 'profesional  (KI 20–24)',         ' 5 soal', '3 soal', 'Permendiknas 16/2007 — adaptasi'],
                ['---',   '---',                             '---',     '---',    '---'],
                ['Guru',  'perilaku_harian   (Komp.1 MP1)', '11 soal', '11 soal', 'MP1 PKG — verbatim (tidak berubah)'],
                ['Guru',  'hubungan_sejawat  (Komp.2 MP1)', '10 soal', '10 soal', 'MP1 PKG — verbatim (tidak berubah)'],
                ['Guru',  'profesional_guru  (Komp.3 MP1)', ' 9 soal', ' 9 soal', 'MP1 PKG — verbatim (tidak berubah)'],
            ]
        );
        $this->command->info("Total siswa  : {$siswaCount} soal (berkurang dari 25 → {$siswaCount}, hemat " . (25 - $siswaCount) . " soal per guru)");
        $this->command->info("Total guru   : {$guruCount} soal (tetap sama)");
        $this->command->info('');
